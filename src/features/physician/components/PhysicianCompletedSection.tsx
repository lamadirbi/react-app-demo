...
export function PhysicianCompletedSection({ items, loading, error }: Props) {
  ...
  return (
    <section id="physician-completed" className="mt-10 scroll-mt-28">
      <div className="mb-3 text-sm font-semibold text-zinc-900">{t("physicianCompletedTitle")}</div>
      <p className="mb-3 text-xs text-(--muted)">{t("physicianCompletedDesc")}</p>
      <div className="grid gap-3">
        ...
        {!loading && !error && items.length === 0 ? (
          <Alert variant="info">{t("noCompletedCases")}</Alert>
        ) : null}
